      ...
      <section className="mb-6">
        ...
        <div className="flex gap-4">
          {['전체', 'twitch', 'youtube'].map((platform) => (
            <button
              key={platform}
              onClick={() => setSelectedPlatform(platform === '전체' ? null : platform)}
      ...
      <section className="mb-6">
        ...
        <div className="flex gap-4">
          {['전체', 'male', 'female'].map((gender) => (
            <button
              key={gender}
              onClick={() => setSelectedGender(gender === '전체' ? null : gender)}
              className={`px-4 py-1 rounded-full border text-sm capitalize ${
      ...
      <section className="mt-10">
        {results.length > 0 && (
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
            {results.map((s) => (
              <div key={s.id} className="border p-4 rounded-xl shadow hover:shadow-lg transition">
                <Image
                ...
                <div className="text-sm text-gray-500 mb-2">
                  🎮 {s.platform.toUpperCase()} &nbsp; | &nbsp; 🚻 {s.gender}
                </div>
                <a
                  href={s.channel_url}
                  target="_blank"
        ...
        {results.length === 0 && !loading && (
          <p className="text-center text-gray-500 mt-8">조건에 맞는 스트리머가 없습니다.</p>
        )}
      </section>
    </main>
  )
