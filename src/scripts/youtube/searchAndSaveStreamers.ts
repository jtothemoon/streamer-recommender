  ...
  for (const [gameType, keywords] of Object.entries(gameKeywords)) {
    ...
    for (const keyword of keywords) {
      ...
      for (const channel of channels) {
        ...
        const subscriberCount = channelDetail.data.items[0]?.statistics?.subscriberCount;
        const hiddenSubscriberCount = channelDetail.data.items[0]?.statistics?.hiddenSubscriberCount;
        ...
        if (subscribers < 1000) {
          console.log(`🚫 구독자 수 미달: ${snippet.title} (${subscribers}명)`);
          continue;
        }
