...
export async function updateExistingStreamers() {
  ...
  type ChannelDetailsResult = 
    | { success: true; data: { subscribers: number; latestUploadDate: Date | null; profileImage: string; description: string } }
  ...
  const main = async () => {
    ...
    for (const streamer of streamers) {
      ...
      if (streamer.platform !== "youtube") {
        console.log(`⏩ YouTube 채널 아님: ${streamer.name} (${streamer.platform})`);
        continue;
      }
